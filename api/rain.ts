...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const repeatedParams = ['field_id', 'lat', 'lon', 'tz', 'asOf', 'start_date', 'end_date'] as const;
    for (const param of repeatedParams) {
      const parsed = readSingleQueryParam(req.query as Record<string, unknown>, param);
      if (!parsed.ok) {
        return badRequest(res, 'Invalid query parameter', parsed.detail);
      }
    }

    const fieldIdQuery = readSingleQueryParam(req.query as Record<string, unknown>, 'field_id');
    const fieldIdRaw =
      (fieldIdQuery.ok ? fieldIdQuery.value : undefined) ||
      (typeof req.body?.field_id === 'string' ? req.body.field_id : undefined);
    ...
    if (req.method === 'POST') {
      ...
      const latParam = readSingleQueryParam(req.query as Record<string, unknown>, 'lat');
      const lonParam = readSingleQueryParam(req.query as Record<string, unknown>, 'lon');
      const parsedLat = parseOptionalFloat(latParam.ok ? latParam.value : undefined);
      const parsedLon = parseOptionalFloat(lonParam.ok ? lonParam.value : undefined);
    ...
    const tzParam = readSingleQueryParam(req.query as Record<string, unknown>, 'tz');
    const tz = (tzParam.ok ? tzParam.value : undefined)?.trim() || 'UTC';
    if (!isValidTimezone(tz)) {
      return badRequest(res, 'Invalid timezone', 'tz must be a valid IANA timezone.');
    }

    const asOfParam = readSingleQueryParam(req.query as Record<string, unknown>, 'asOf');
    const asOf = (asOfParam.ok ? asOfParam.value : undefined)?.trim();
    if (asOf && !isValidIsoDateTime(asOf)) {
      return badRequest(res, 'Invalid asOf', 'asOf must be a valid ISO 8601 datetime.');
    }

    const startDateParam = readSingleQueryParam(req.query as Record<string, unknown>, 'start_date');
    const endDateParam = readSingleQueryParam(req.query as Record<string, unknown>, 'end_date');
    const startDate = (startDateParam.ok ? startDateParam.value : undefined)?.trim();
    const endDateQuery = (endDateParam.ok ? endDateParam.value : undefined)?.trim();
    ...
    if (startDate) {
        ...
        lat !== undefined && lon !== undefined
          ? fetchIemCustomRange(lat, lon, startDate, endDate).then(
              ...
              (err: unknown) => ({
                ok: false as const,
                result: null,
                error: err instanceof IemFetchError ? err : null,
              })
            )
          : Promise.resolve({ ok: false as const, result: null, error: null });
      ...
      if (radarOutcome.ok && radarOutcome.result?.hasUsableData) {
        sourceStatus.radar = 'ok';
        total = radarOutcome.result.total;
        if (radarOutcome.result.dataWarning) {
          warnings.push(radarOutcome.result.dataWarning);
        }
      }
    ...
    let radarDataWarning: string | undefined;
    if (lat !== undefined && lon !== undefined) {
      try {
        const dates = getRequiredDates(keys);
        const iemResult = await fetchIemMultipleDays(lat, lon, dates);
        if (iemResult.hasUsableData) {
          iemTotals = aggregateRain(iemResult.hourMap, keys);
          sourceStatus.radar = 'ok';
          radarDataWarning = iemResult.dataWarning;
        }
      } catch (err: unknown) {
    ...
    const warnings: string[] = [];
    if (radarDataWarning) warnings.push(radarDataWarning);
